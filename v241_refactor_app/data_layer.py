...
def get_stock_data(ticker: str, token: str) -> Tuple[pd.Series, pd.Series, List[int]]:
    ...
    params = {
        ...
        "columns": "date,adjClose,divCash,close",
    }
    ...
    if "adjClose" in df.columns and _safe_numeric_series(df["adjClose"]).notna().any():
        price_series = _safe_numeric_series(df["adjClose"])
    elif "close" in df.columns and _safe_numeric_series(df["close"]).notna().any():
        price_series = _safe_numeric_series(df["close"])
    else:
        raise ValueError(f"No usable price column returned for '{ticker}'.")

    div_series = _safe_numeric_series(df["divCash"]) if "divCash" in df.columns else pd.Series(0.0, index=df.index)
    daily = pd.DataFrame({"adj_price": price_series, "divCash": div_series}, index=df.index).dropna(subset=["adj_price"])
    daily = daily[daily["adj_price"] > 0]
    if daily.empty:
        raise ValueError(f"No positive adjusted prices returned for '{ticker}'.")

    monthly = daily.resample("ME").agg({"adj_price": "last", "divCash": "sum"})
    monthly["total_return"] = monthly["adj_price"].pct_change(fill_method=None)
    monthly["dividend_yield"] = (monthly["divCash"] / monthly["adj_price"].shift(1)).replace([np.inf, -np.inf], np.nan).fillna(0.0)
